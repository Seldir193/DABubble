    ...
    if (senderId && this.recipientId) {
        ...
        this.messageService.listenForPrivateMessages(conversationId, (messages: Message[]) => {
          this.privateMessages = messages.map((msg: Message) => ({
            ...
            // Starte Live-Listener für Antworten
...
//openThreadEvent(msg: any): void {
  //this.openThread.emit(msg); // Ereignis auslösen
  //console.log('Thread geöffnet für Nachricht:', msg);
//}
...
openThreadEvent(msg: any): void {
  ...
  // Nachrichten-Listener abonnieren
  this.unsubscribeFromThreadMessages = this.threadService.getThreadMessages(msg.id, (messages) => {
    ...
    this.privateMessages = this.privateMessages.map((message) =>
      message.id === msg.id
        ? {
            ...message,
            replies: messages,
            lastReplyTime: latestMessage?.timestamp || message.lastReplyTime,
          }
        : message
    );
  });

  // Thread-Details-Listener abonnieren
  this.unsubscribeFromThreadDetails = this.threadService.listenToThreadDetails(msg.id, (threadData) => {
    const lastResponseTime = threadData?.threadLastResponseTime
      ? new Date(threadData.threadLastResponseTime.toDate())
      : null;
    ...
    this.privateMessages = this.privateMessages.map((message) =>
      message.id === msg.id
        ? {
            ...message,
            threadLastResponseTime: lastResponseTime || message.threadLastResponseTime,
          }
        : message
    );
  });
...
loadReplyCounts(): void {
  const threadIds = this.privateMessages.map((msg) => msg.id);

  if (threadIds.length > 0) {
    this.threadService.getReplyCountsForMessages(threadIds).then((replyCounts) => {
      this.privateMessages.forEach((msg) => {
        msg.replyCount = replyCounts[msg.id] || 0; // Setze 0, wenn keine Antworten vorhanden sind
      });
    }).catch((err) => {
...
startLiveReplyCountUpdates(): void {
  ...
  if (threadIds.length > 0) {
    this.threadService.loadReplyCountsLive(threadIds, (replyCounts) => {
      this.privateMessages.forEach((msg) => {
        msg.replyCount = replyCounts[msg.id]?.count || 0; // Antwortanzahl aktualisieren
        msg.threadLastResponseTime = replyCounts[msg.id]?.lastResponseTime || msg.timestamp; // Letzte Antwortzeit setzen
      });
    });
  }
}
...
ngOnChanges(changes: SimpleChanges): void {
  // Änderungen an recipientId
  if (changes['recipientId'] && !changes['recipientId'].isFirstChange()) {
    console.log('Empfänger-ID hat sich geändert:', changes['recipientId'].currentValue);
    ...
    // Lade Daten für den neuen Empfänger
    this.loadRecipientData();
  ...
  if (changes['threadData'] && changes['threadData'].currentValue) {
    ...
    // Verarbeite die aktualisierten Thread-Daten
