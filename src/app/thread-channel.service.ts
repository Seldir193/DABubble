...
export class ThreadChannelService {
  ...
  async addThreadReply(channelId: string, threadId: string, reply: any): Promise<void> {
    ...
    try {
      ...
      // **Aktualisiere `replyCount` in Firestore**
      await updateDoc(threadDocRef, { threadLastResponseTime: serverTimestamp() });
  
      console.log('✅ Antwort erfolgreich gespeichert und `replyCount` aktualisiert.');
    } catch (error) {
...
public convertToDate(timestamp: any): Date | null {
  ...
  async updateReplyCount(channelId: string, threadId: string, count: number): Promise<void> {
    const threadDocRef = doc(this.firestore, `channels/${channelId}/threads/${threadId}`);
  
    try {
      await updateDoc(threadDocRef, { replyCount: count });
      console.log(`✅ Reply-Count für Thread ${threadId} auf ${count} aktualisiert.`);
    } catch (error) {
      console.error(`❌ Fehler beim Aktualisieren des Reply-Counts:`, error);
    }
  }
